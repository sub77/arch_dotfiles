...
prompt_context() {
        if [[ -n "$SSH_CLIENT" && "$USER" == "root" ]]; then
                prompt_segment 236 10 "%(!.%{%F{2}%}.)$USER%{%F{4}%}@%{%F{15}%}%M"
        elif [[ -n "$SSH_CLIENT" ]]; then
                prompt_segment 236 10 "%(!.%{%F{10}%}.)$USER%{%F{4}%}@%{%F{15}%}%M"
        elif [[ -z "$SSH_CLIENT" && "$USER" == "root" ]]; then
                prompt_segment 236 10 "%(!.%{%F{1}%}.)$USER%{%F{4}%}@%{%F{15}%}%m"
        else
                prompt_segment 236 10 "%(!.%{%F{10}%}.)$USER%{%F{4}%}@%{%F{15}%}%m"
        fi
}
...
prompt_newline() {
        ...
        [[ $UID -ne 0 ]] && symbols+="%{%F{15}%}$"
        echo "$symbols"
}

prompt_time() {
        fqdn=$(hostname -f)
        if [[ $fqdn == kinetik.* ]]; then
                prompt_segment 1 15 "%D{%H:%M:%S}"
        elif [[ $fqdn == video.* ]]; then
                prompt_segment 2 15 "%D{%H:%M:%S}"
        elif [[ $fqdn == licht.* || $fqdn == ton.* ]]; then
                prompt_segment 3 15 "%D{%H:%M:%S}"
        elif [[ $fqdn == monitor.* ]]; then
                prompt_segment 4 15 "%D{%H:%M:%S}"
        elif [[ $fqdn == workstation.* || $fqdn == mobile.* ]]; then
                prompt_segment 6 15 "%D{%H:%M:%S}"
        else
