...
alias pacmir='sudo armrr'

# Pacman - https://wiki.archlinux.org/index.php/Pacman_Tips
alias pacupgrd='sudo pacman -Syu'        # Synchronize with repositories before upgrading packages that are out of date on the local system.
...
paclist() {
  LC_ALL=C pacman -Qein $(pacman -Qu|cut -d" " -f 1)|awk ' BEGIN {FS=":"}/^Name/{printf("\033[1;36m%s\033[1;37m", $2)}/^Description/{print $2}'
}
paclist-all() {
  LC_ALL=C pacman -Qei $(pacman -Qu|cut -d" " -f 1)|awk ' BEGIN {FS=":"}/^Name/{printf("\033[1;36m%s\033[1;37m", $2)}/^Description/{print $2}'
}
aurlist() {
  LC_ALL=C pacman -Qeim $(pacman -Qu|cut -d" " -f 1)|awk ' BEGIN {FS=":"}/^Name/{printf("\033[1;36m%s\033[1;37m", $2)}/^Description/{print $2}'
}
